...
def split_lead_rhythm_guitar(
    ...
    *,
    ...
    Always writes lead_guitar.wav + rhythm_guitar.wav when a guitar stem exists
    (best-effort even when confidence gates fail). Always writes
    guitar_split_diagnostics.json. Spectral candidates never emit unless
        ...
        if non_spectral:
            ...
            if forced is None:
                diag = LeadRhythmDiagnostics(
                    ...
                    method=best.method,
                    features={"quarantined": True, "detail": best.detail},
                )
                diag.write_json(diag_path)
                return {"guitar_split_diagnostics": diag_path}, diag
            best = forced
    ...
    if forced:
        ...
        reason = (
            ...
            f"({' ; '.join(reasons)}); labels may be inaccurate"
        )
