...
class IsolateConfig:
    ...
    # automatically by separate_stems() whenever a guitar stem exists.
    # Kept only so existing callers passing these flags don't break.
    ...
    *,
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if "guitar" not in artifacts:
            ...
            if lead_rhythm_diag.outcome == "lead_rhythm":
                progress("guitar_split", lead_rhythm_diag.reason)
            else:
                progress(
                    "guitar_split", f"Lead/Rhythm not applied: {lead_rhythm_diag.reason}"
                )
        ...
        if (
            ...
            # Always treat derived Lead/Rhythm as selected-by-default presence,
            # even on best-effort / low-confidence emits.
            for derived in ("lead_guitar", "rhythm_guitar"):
                if derived in presence:
                    ...
                    presence[derived] = StemPresence(
                        ...
                        reason=(
                            "Lead/Rhythm always emitted from guitar stem"
                            + (
