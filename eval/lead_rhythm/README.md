...
best-effort under the always-emit policy).
...
| Mono or highly correlated L/R single guitar | Still emit Lead+Rhythm (best-effort / low confidence); combined Guitar kept |
| Centered overlapping lead+rhythm (dead center, L≡R) | Still emit best-effort Lead+Rhythm (`forced_emit` / `low_confidence`); labels may be wrong — no trained separator |
| Double-tracked wide rhythm (same part, mild pan, same pitch) | Still emit best-effort; spatial pitch-divergence reject may fall through to midside/register |
| Intermittent lead over continuous rhythm | Emit; confident if margin OK |
| Register-separated mono dual parts | May emit via `method=register` |
| 4-stem model (no guitar) | skipped; no lead/rhythm |
| Demucs guitar bleed | May lower confidence → forced emit with warning |
...
- **Ambiguous rate on gt≤1** — fraction correctly refused
...
**2026-07-17 update — always-emit + Mid/Side:** whenever a `guitar` stem
exists, `lead_guitar.wav` + `rhythm_guitar.wav` are always written. Confidence
gates still set `forced_emit` / `low_confidence` when evidence is weak.
Mid/Side candidate targets centered lead + wide-panned rhythm (common mix
layout). Detected Instruments always default-checks Lead Guitar and Rhythm
Guitar when those WAVs exist.

## Manual procedure (full mix → Demucs → split)

```bash
export PYTHONPATH=src:.
audio-isolate --audio /path/to/clip.wav --output ./eval/lead_rhythm/out --model htdemucs_6s --quality fast --lead-rhythm
