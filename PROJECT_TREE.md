...
Both products import `src/audio_to_tab/` (Demucs isolate, YouTube ingest, tab PDF pipeline). They do **not** share UI or job runners.
...
| Demucs / guitar stem | `src/audio_to_tab/isolate.py` (`IsolateConfig`, `separate_stems`, segment clamp), `src/audio_to_tab/separate.py` (Demucs CLI / guitar-ft) |
...
| Isolate job queue (desktop) | `ui/isolate_jobs.py` (serial worker, status on disk), queue UI in `ui/pages/isolate.py` |
...
| Tab PDF | Engine: `src/audio_to_tab/pipeline.py`, `transcribe.py`, `tab_generate.py`, `pdf_render.py`. Desktop: `ui/pages/tab_pdf.py`. Website: `web/src/pages/TabPage.tsx` + tab job in `backend/jobs/runner.py` |
...
| Tests for isolation | `tests/test_isolate.py`, `tests/test_isolate_jobs.py`, `tests/test_isolate_state.py`, `tests/test_ingest.py`, `tests/test_mixer.py`, `tests/test_stem_mixer_component.py` |
...
- `pyproject.toml` — package `audio-to-tab-pdf`, Python 3.10–3.12, extras `dev` / `eval` / `demucs`.
- `requirements.txt` — pip pin of runtime (Streamlit, FastAPI, yt-dlp, …).
...
- `.gitignore` — venvs, `.env`, wavs, internal docs including this file.
...
- `audio_to_tab/isolate.py` — Demucs multi-stem isolate, region trim, presence / bass-bleed diagnostics, `--segment` clamp.
- `audio_to_tab/separate.py` — Demucs subprocess / frozen in-process; optional guitar-ft weights.
...
- `audio_to_tab/pipeline.py` — audio → tab PDF end-to-end.
...
- `audio_to_tab/cli/isolate.py` — CLI isolate.
...
- `pages/isolate.py` — Audio Isolation page (upload, YouTube, queue, mixer, downloads).
- `pages/tab_pdf.py` — Tab PDF page.
- `isolate_jobs.py` — serial Demucs worker; job status JSON under app data dir.
- `isolate_state.py` — presets, custom stems, progress stages (pure helpers).
...
- `main.py` — FastAPI routes: auth, upload, isolate/tab jobs, stem/mix download, WS events.
...
- `jobs/manager.py` — persist jobs, artifacts, cancel.
- `jobs/runner.py` — runs `separate_stems` / `run_pipeline`.
...
- `test_isolate.py` — isolate engine (Demucs mocked), including `--segment` clamp.
- `test_isolate_jobs.py` — desktop serial queue / remove / error formatting.
- `test_isolate_state.py` — presets and progress helpers.
...
- `prewarm.py` — pull Demucs / Basic Pitch weights (API first-request).
...
On disk: `oracle-free-memory-spike.md` (gitignored). Not in public git.
...
- Do not commit this file; it is gitignored like `PROJECT_STRUCTURE.md`.
