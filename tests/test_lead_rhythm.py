...
def test_split_lead_rhythm_mono_still_emits_best_effort(tmp_path: Path):
    """Mono guitar still gets Lead/Rhythm WAVs (best-effort / low confidence)."""
    sr = 44100
    t = np.linspace(0, 1.0, sr, endpoint=False)
    mono = np.sin(2 * np.pi * 440 * t)
    guitar = tmp_path / "guitar.wav"
...
def test_spectral_emit_quarantined_falls_back_to_musical_pair(tmp_path: Path):
    ...
    extra, diag = split_lead_rhythm_guitar(
        guitar, tmp_path, use_basic_pitch=False, thresholds=thr
    )
    assert "lead_guitar" in extra and "rhythm_guitar" in extra
    assert diag.outcome == "lead_rhythm"
...
def test_split_midside_centered_lead_wide_rhythm(tmp_path: Path):
    ...
    guitar = tmp_path / "guitar.wav"
    _write_stereo(guitar, left, right, sr)
    # Hard-pan spatial may also admit; either spatial or midside is fine.
    extra, diag = split_lead_rhythm_guitar(guitar, tmp_path, use_basic_pitch=False)
    assert diag.outcome == "lead_rhythm"
    assert "lead_guitar" in extra and "rhythm_guitar" in extra
...
def test_default_isolate_selected_stems_always_checks_lead_rhythm():
    from ui.pages.isolate import default_isolate_selected_stems

    presence = {
        "guitar": {"present": True},
        "lead_guitar": {"present": False},
        "rhythm_guitar": {"present": False},
        "vocals": {"present": True},
    }
    selected = default_isolate_selected_stems(
        ["vocals", "guitar", "lead_guitar", "rhythm_guitar"], presence
    )
